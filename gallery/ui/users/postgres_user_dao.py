...
class PostgresUserDAO(UserDAO):
    ...
    def add_image(self, filename, username):
        db.execute("insert into images values (%s,%s);",(filename,username))

    def get_image_names(self, username):
        # named the columsn wrong, too lazy to change
        result = []
        cursor = db.execute("select filename from images where username=%s;",(username,))
        for t in cursor.fetchall():
            result.append(t[0])
        return result
